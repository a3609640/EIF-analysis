...
EIF.RNAseq.data <- getProfileData(mycgds,
                                  EIF.Onco,
                                  "laml_tcga_pan_can_atlas_2018_rna_seq_v2_mrna",
                                  "laml_tcga_pan_can_atlas_2018_all")
...
getCNV <- function(x) {
  CNV <- getProfileData(mycgds,
                        x,
                        "laml_tcga_pan_can_atlas_2018_gistic",
                        "laml_tcga_pan_can_atlas_2018_all")
  v <- rownames(CNV)
...
plot.CNV.RNAseq <- function(geneCNV, RNAseq) {
  ...
  ### CNV.data <- getCNV("BRAF")
  CNV.data <- getCNV(Onco.gene)
  CNV.DNFA.RNAseq <- cbind(CNV.data, EIF.RNAseq.data)
